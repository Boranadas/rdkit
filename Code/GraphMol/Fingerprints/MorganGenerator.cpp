...
std::uint32_t MorganAtomEnv::getBitId(
  ...
  std::cout << d_layer << " " << *dp_lastLayer << std::endl;

  if (d_layer != *dp_lastLayer) {
    *dp_lastLayer = d_layer;
    std::copy(dp_nextAtomInvariants->begin(), dp_nextAtomInvariants->end(),
              dp_currentAtomInvariants->begin());
  }

  if (d_layer == 0) {
    return (*atomInvariants)[d_atomId];
  } else {
    std::vector<std::pair<int32_t, uint32_t>> sortedNeighbors;
    const Atom *atom = dp_mol->getAtomWithIdx(d_atomId);
    ROMol::OEDGE_ITER beg, end;
    boost::tie(beg, end) = dp_mol->getAtomBonds(atom);
    while (beg != end) {
      ...
      sortedNeighbors.push_back(std::make_pair(
          bt, (*dp_currentAtomInvariants)[bond->getOtherAtomIdx(d_atomId)]));

      beg++;
    }
    std::sort(sortedNeighbors.begin(), sortedNeighbors.end());

    bool looksChiral = (atom->getChiralTag() != Atom::CHI_UNSPECIFIED);
    boost::uint32_t invar = d_layer;
    gboost::hash_combine(invar, (*dp_currentAtomInvariants)[d_atomId]);
    for (std::vector<std::pair<int32_t, uint32_t>>::const_iterator it =
             sortedNeighbors.begin();
         it != sortedNeighbors.end(); ++it) {
      ...
      if (morganArguments->df_includeChirality && looksChiral &&
          (*dp_chiralAtoms)[d_atomId]) {
        if (it->first != static_cast<int32_t>(Bond::SINGLE)) {
          looksChiral = false;
        } else if (it != sortedNeighbors.begin() &&
    ...
    (*dp_nextAtomInvariants)[d_atomId] = invar;

    if (dp_deadEnvironments) {
      BOOST_FOREACH (MorganAtomEnv *de, *dp_deadEnvironments) {
        de->getBitId(arguments, atomInvariants, bondInvariants, nullptr);
      }
...
MorganAtomEnv::MorganAtomEnv(const std::uint32_t code,
                             ...
                             boost::dynamic_bitset<> *chiralAtoms,
                             unsigned int *lastLayer,
                             std::vector<MorganAtomEnv *> *deadEnvironments,
                             bool ownsAllData)
    : d_code(code),
      d_atomId(atomId),
      d_layer(layer),
      dp_mol(mol),
      dp_currentAtomInvariants(currentAtomInvariants),
      dp_nextAtomInvariants(nextAtomInvariants),
      dp_chiralAtoms(chiralAtoms),
      dp_lastLayer(lastLayer),
      dp_deadEnvironments(deadEnvironments),
      df_ownsAllData(ownsAllData) {}

MorganAtomEnv::~MorganAtomEnv() {
  if (df_ownsAllData) {
    delete dp_currentAtomInvariants;
    delete dp_nextAtomInvariants;
    delete dp_chiralAtoms;
  }
  if (dp_deadEnvironments) {
    BOOST_FOREACH (MorganAtomEnv *de, *dp_deadEnvironments) { delete de; }
    delete dp_deadEnvironments;
  }
}
...
std::vector<AtomEnvironment *> MorganEnvGenerator::getEnvironments(
  ...
  for (unsigned int i = 0; i < nAtoms; ++i) {
    if (includeAtoms[i]) {
      if (!morganArguments->df_onlyNonzeroInvariants || currentInvariants[i]) {
        result.push_back(new MorganAtomEnv(
            currentInvariants[i], i, 0, &mol, currentAtomInvariants,
            nextAtomInvariants, chiralAtomsP, lastLayer));
      }
    }
  }
  ...
  for (unsigned int layer = 0; layer < morganArguments->d_radius; ++layer) {
    ...
    for (std::vector<AccumTuple>::const_iterator iter =
      ...
      if (std::find(neighborhoods.begin(), neighborhoods.end(),
          ...
          if (includeAtoms[iter->get<2>()]) {
            result.push_back(
                new MorganAtomEnv(iter->get<1>(), iter->get<2>(), layer + 1,
                                  &mol, currentAtomInvariants,
                                  nextAtomInvariants, chiralAtomsP, lastLayer));
            neighborhoods.push_back(iter->get<0>());
            roundInvariants[iter->get<2>()] = iter->get<1>();
          }
        }
      } else {
        // we have seen this exact environment before, this atom
        ...
        roundInvariants[iter->get<2>()] = iter->get<1>();
        deadEnvThisRound->push_back(
            new MorganAtomEnv(iter->get<1>(), iter->get<2>(), layer + 1, &mol,
                              currentAtomInvariants, nextAtomInvariants,
                              chiralAtomsP, lastLayer));
      }
    }

    if (!result.empty()) {
      MorganAtomEnv *e = dynamic_cast<MorganAtomEnv *>(result.back());
      if (e->dp_deadEnvironments) {
        BOOST_FOREACH (MorganAtomEnv *de, *deadEnvThisRound) {
          e->dp_deadEnvironments->push_back(de);
        }
      } else {
        e->dp_deadEnvironments = deadEnvThisRound;
      }
    } else {
      BOOST_FOREACH (MorganAtomEnv *de, *deadEnvThisRound) { delete de; }
      delete deadEnvThisRound;
    }

    std::cout << "refresh0000" << std::endl;
  ...
  if (result.empty()) {
    delete currentAtomInvariants;
    delete nextAtomInvariants;
    delete chiralAtomsP;
  } else {
    MorganAtomEnv *e = dynamic_cast<MorganAtomEnv *>(result.back());
    e->df_ownsAllData = true;
  }
